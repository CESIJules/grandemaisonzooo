(function() {
  ...
  async function loadArtists() {
    ...
    try {
      const [artistsResponse, postsResponse] = await Promise.all([
        ...
        fetch('/api/posts?artist=Tous').catch(() => null)
      ]);
  ...
  async function fetchAndSetProgress(rawTitle, isInitialLoad = false) {
    ...
    try {
      const response = await fetch(`/api/track/current?nocache=${new Date().getTime()}`);
  ...
  async function resyncProgress(rawTitle) {
    try {
      ...
      if (response.ok && data.duration && data.start_time && data.server_now) {
        ...
        if (data.filename) {
          const serverTrack = data.filename.replace(/\.[^/.]+$/, "").toLowerCase();
  ...
  async function loadAllMetadata() {
    ...
    try {
      const response = await fetch(`/api/music/all-metadata?nocache=${Date.now()}`);
  ...
  async function fetchPlayHistory() {
    ...
    try {
      const response = await fetch(`/api/track/history?limit=3&nocache=${Date.now()}`);
      const data = await response.json();
  ...
  function updateCoverUI(filename) {
      ...
      const newSrc = `/api/music/cover?file=${encodeURIComponent(filename)}`;
  ...
  async function fetchCurrentSong() {
    ...
    try {
      ...
      const [trackResponse, icecastResponse] = await Promise.all([
        fetch(`/api/track/current?nocache=${Date.now()}`),
  ...
  async function renderTimelinePosts(artist = 'Tous') {
    ...
    const fetchURL = `/api/posts?artist=${encodeURIComponent(artist)}`;
