(async function() {
    ...
    async function loadAllMetadata() {
        try {
            const response = await fetch('/api/music/all-metadata');
            if (response.ok) {
                const data = await response.json();
    ...
    async function getMusicMetadata(filename, force = false) {
        ...
        try {
            const response = await fetch('/api/music/analyze-metadata', {
                method: 'POST',
    ...
    async function skipSong() {
        ...
        try {
            const response = await fetch('/api/track/skip', {
                method: 'POST',
                cache: 'no-store'
            });
    ...
    async function updatePost(formData) {
        try {
            const postId = formData.get('id') || formData.get('editingPostId');
    ...
    async function downloadYoutube(url) {
        ...
        try {
            const response = await fetch('/api/music/download/youtube', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    async function downloadSpotify(url) {
        ...
        try {
            const response = await fetch('/api/music/download/spotify', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ url: url })
            });
    ...
    async function bulkDownloadYoutube(urls) {
        ...
        for (const url of validUrls) {
            ...
            const endpoint = isSpotify ? '/api/music/download/spotify' : '/api/music/download/youtube';
    ...
    async function renderMusicFiles(filter = '', forceRefresh = false) {
        try {
            ...
            if (forceRefresh || allMusicFiles.length === 0) {
                const response = await fetch('/api/music/files', { cache: 'no-store' });
                if (!response.ok) throw new Error(`Erreur serveur: ${response.statusText}`);
                const result = await response.json();
            ...
            filteredFiles.forEach(file => {
                ...
                const coverUrl = `/api/music/cover?file=${encodeURIComponent(file)}`;
    ...
    async function renameMusicFile(oldFilename) {
        ...
        try {
            const response = await fetch('/api/music/rename', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    async function savePlaylistChanges() {
        ...
        try {
            const response = await fetch('/api/playlists/' + encodeURIComponent(currentEditingPlaylist.name), {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
    ...
    async function fetchArtistProfiles() {
        try {
            const response = await fetch('/api/artists/profiles', { cache: 'no-store' });
    ...
    async function saveArtistProfile(formData) {
        ...
        if (imageFile && imageFile.size > 0) {
            ...
            try {
                const uploadRes = await fetch('/api/uploads/artist-image', { method: 'POST', body: uploadData });
    ...
    async function loadAnalyticsHeader() {
        try {
            const res = await fetch('/api/analytics?type=stats_header');
    ...
    window.loadAudienceChart = async function(range) {
        try {
            const res = await fetch(`/api/analytics?type=audience&range=${range}`);
            const json = await res.json();
    ...
    async function loadTopLists() {
        // Top Tracks
        try {
            const res = await fetch('/api/analytics?type=top_tracks');
            const json = await res.json();
        ...
        // Top Artists
        try {
            const res = await fetch('/api/analytics?type=top_artists');
            const json = await res.json();
        ...
        editorCoverInput.addEventListener('change', (e) => {
            ...
            if (file && currentEditingPlaylist) {
                ...
                reader.onload = async (event) => {
                    ...
                    // Save cover via API
                    try {
                        const response = await fetch('/api/playlists/cover', {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json' },
                            body: JSON.stringify({
    ...
    function renderNewPlaylistSongs() {
        ...
        currentEditingPlaylist.songs.forEach((songPath, index) => {
            const filename = songPath.split('/').pop();
            const coverUrl = `/api/music/cover?file=${encodeURIComponent(filename)}`;
    ...
    async function autoSavePlaylist() {
        ...
        saveTimeout = setTimeout(async () => {
            try {
                ...
                const playlistUrlName = encodeURIComponent(originalPlaylistName || currentEditingPlaylist.name);
                const response = await fetch('/api/playlists/' + playlistUrlName, {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
